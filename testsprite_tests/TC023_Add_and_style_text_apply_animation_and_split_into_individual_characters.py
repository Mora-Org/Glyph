...
async def run_test():
    ...
    try:
        ...
        # -> Fill the project name field and click 'Criar Projeto' to create a new project so the editor opens.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/form/div/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('LetteringSplitTest Project')
        ...
        # -> Open the Lettering panel by clicking the '✦ Lettering' control in the header.
        frame = context.pages[-1]
        # Click element
        ...
        # -> Click the text element (or its editor) to edit the text content so it can be changed to 'LetteringSplitTest'.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/div/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Change the font family via the font dropdown (index 426) so typography change is applied.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Select an animation effect (Tremor), click the 'Split em letras individuais' control, then extract the UI to verify the text was split into individual characters and the Lettering panel remains open.
        frame = context.pages[-1]
        # Click element
